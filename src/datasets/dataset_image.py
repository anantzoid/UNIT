...
class dataset_image(data.Dataset):
  def __init__(self, specs):
    ...
    self.target_size = specs['target_size']
    self.crop_image_height = specs['crop_image_height']
    self.crop_image_width = specs['crop_image_width']

    self.img_folder = os.path.join(self.root, self.folder)
    self.images = [os.path.join(self.img_folder, img) for img in os.listdir(self.img_folder)]
    np.random.shuffle(self.images)
    self.dataset_size = len(self.images)

  def __getitem__(self, index):
    data = self._load_one_image(self.images[index])
    return data
  ...
  def _load_one_image(self, img_name, test=False):
    ...
    h, w= img.shape
    if self.target_size != w or self.target_size != h: 
    ...
    crop_img = (crop_img - np.max(crop_img)) / np.mean(crop_img)
    crop_img = np.reshape(crop_img, (1, crop_img.shape[0], crop_img.shape[1]))
    return crop_img
