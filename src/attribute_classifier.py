...
train_loader = get_data_loader(config.datasets['train'], batch_size, num_workers=6)
val_loader = get_data_loader(config.datasets['val'], batch_size, num_workers=4)
# Model
if args.resume:
    # Load checkpoint.
...
def train(epoch):
    ...
    optimizer = optim.Adam(model.parameters(), lr=lr, betas=(0.5, 0.999), weight_decay=1e-6)
