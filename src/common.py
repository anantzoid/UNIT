...
def get_data_loader(conf, batch_size, njobs, _conf):
  dataset = []
  #print("dataset=%s(conf)" % conf['class_name'])
  #exec ("dataset=%s(conf)" % conf['class_name'])
  #print(len(dataset))
  dataset = dataset_image(_conf)
  return torch.utils.data.DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, num_workers=njobs)
